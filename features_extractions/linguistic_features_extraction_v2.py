...
def main():
	...
	df = pd.read_csv(args.input_file,sep='|',names=['utterance','text'])
	df["book"] = ['boule de suif' for i in np.arange(len(df))]
	print(df.head())
	with tqdm(total=len(list(df.iterrows()))) as pbar:
		for i, row in df.iterrows():
			pbar.update(1)
			text=df.loc[i]['text']
			uttFilename=row['utterance']
			label=row['book']
			print(text)
			sentparser.parsing_sentence(text,uttFilename,label)
			print('done')
		
		sentparser.pprint(args.out_path_file)
			# 
