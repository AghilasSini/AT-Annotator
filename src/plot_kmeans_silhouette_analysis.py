...
def main():
    ...
    parser.add_argument("-s", "--sep",  default="\t",
                        help="define specific seperator")
    # Add arguments
    parser.add_argument("inFilename", help="The input file to be projected")
    parser.add_argument("outFilename", help="The input file to be projected")


    # Parsing arguments
    args = parser.parse_args()

    df = pd.read_csv(args.inFilename,sep=args.sep)
    ...
    range_n_clusters =np.arange(5,6,+1)
    for n_clusters in range_n_clusters:
        ...
        df['label_predict']=[label_dict[lb] for lb in  cluster_labels]
        ...
        for i, c in enumerate(centers):
            ax2.scatter(c[0], c[1], alpha=1,
                        s=50, edgecolor='k',label=label_dict[i])
    ...
    # df[df["corpus"] == corpus_com]



    df.to_csv(args.outFilename,sep=args.sep)
